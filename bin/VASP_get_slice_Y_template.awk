...
BEGIN{
  ...
  plotfile="ps-plot-Y.gnu"

  
  getline < ARGV[1]; title =$0
  getline < ARGV[1]; scale =$0

  getline < ARGV[1]; h1=$1; h2=$2; h3=$3; print > "/dev/stderr"
  getline < ARGV[1]; h4=$1; h5=$2; h6=$3; print > "/dev/stderr"
  getline < ARGV[1]; h7=$1; h8=$2; h9=$3; print > "/dev/stderr"
  ...
  for (x=1-margin;x<=x_grid+margin;x++){
    for (z=1-margin;z<=z_grid+margin;z++){
      ...
      zg=z%z_grid; 
      if(zg==0) zg=z_grid; 
      if(zg<0) zg=z_grid+zg
      
      xx=x*dx; yy=y*dy; zz=z*dz
      printf "%f %f %f\n",xx,zz,grid[xg,Yslice,zg] >> "dens.dat"
    }
  }
  ...
  print "#---------------------------------------------------" >> plotfile
  print "set dgrid3d   "x_grid","z_grid",16" >> plotfile
