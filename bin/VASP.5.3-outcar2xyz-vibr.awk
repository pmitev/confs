...
/POMASS/ {
  if (ntypes) {for (i=1; i<= ntypes; i++) mass[i]=$(i+2);}
}
...
/POSITION                                       TOTAL-FORCE/{
  ...
  for(t=1;t<=ntypes;t++){
    for(i=1;i<=ntype[t];i++){
      ...
      at[j]=typeT[t]; x[j]=$1; y[j]=$2; z[j]=$3; atmass[j]= mass[t];
    }
  }
}
...
/Eigenvectors and eigenvalues of the dynamical matrix/{
  ...
  while ($1 != "Finite") {
    ...
    printf "FREQ N %g %g  cm-1 A IRint 0 RAMAN_int 1",$1,$8
    if(!jmolscriptline) {print " jmolscript: load 1.xyz {1 1 1} unitcell {"a,b,c, alpha, beta, gamma"}; "; jmolscriptline=1;} else {print ""}

    getline;getline;j=0
    while (NF>1){
      j++
      vs= vscale/sqrt(atmass[j])
      print at[j],$1,$2,$3,0, $4*vs,$5*vs,$6*vs
      getline; 
    }
  getline; 
  }
